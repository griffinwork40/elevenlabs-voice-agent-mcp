...
async function testClientConnection(): Promise<Client | null> {
  try {
...
async function testListTools(client: Client): Promise<void> {
  try {
    ...
    recordTest(
      ...
      `Found ${toolCount} tools (expected ${expectedCount})`,
...
async function testToolSchema(client: Client): Promise<void> {
  try {
    const tools = await client.listTools();
    if (!tools.tools || tools.tools.length === 0) {
    ...
    recordTest(
      "Tool Schema",
      hasSchema,
      hasSchema ? "Tool has input schema defined" : "Tool missing input schema"
    );
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
async function testToolExecution(client: Client): Promise<void> {
  try {
    ...
    if (result.content && result.content.length > 0) {
      ...
      recordTest(
        "Tool Execution",
        false,
        "Tool executed but returned no content",
  ...
  try {
    ...
    try {
      ...
      recordTest(
        "Error Handling",
        true,
        "Tool handled invalid input (may return error in content)"
      );
    } catch (error) {
      ...
      recordTest(
        ...
        isExpectedError ? undefined : message
      );
    }
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
async function testToolDescriptions(client: Client): Promise<void> {
  try {
    const tools = await client.listTools();
    if (!tools.tools || tools.tools.length === 0) {
