...
const results: Array<{ tool: string; passed: boolean; message: string; error?: string }> = [];

function recordResult(tool: string, passed: boolean, message: string, error?: string): void {
...
async function testTool(client: Client, test: ToolTest): Promise<void> {
  try {
    ...
    if (test.expectSuccess) {
      if (result.content && result.content.length > 0) {
        ...
        recordResult(
          test.toolName,
          false,
          `${test.description} - No content returned`,
