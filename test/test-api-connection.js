...
async function testApiKeyPresence() {
    const apiKey = process.env.ELEVENLABS_API_KEY;
    if (!apiKey) {
        recordTest("API Key Presence", false, "ELEVENLABS_API_KEY environment variable is not set");
        return;
    }
...
async function testApiKeyFormat() {
    ...
    recordTest("API Key Format", isValid, isValid ? "API key format appears valid" : "API key format may be invalid");
}
async function testNetworkConnectivity() {
    try {
        ...
        recordTest("Network Connectivity", true, `Connected to ${API_BASE_URL} (status: ${response.status})`);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        recordTest("Network Connectivity", false, `Cannot connect to ${API_BASE_URL}`, message);
    }
}
async function testApiAuthentication() {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            recordTest("API Authentication", false, "API key is invalid or expired", `Status: ${response.status}`);
        }
        else {
            recordTest("API Authentication", false, `Unexpected response status: ${response.status}`, JSON.stringify(response.data).substring(0, 100));
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
async function testAgentsEndpoint() {
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            recordTest("Agents Endpoint", false, "Unauthorized - check API key permissions", `Status: ${response.status}`);
        }
        else {
            recordTest("Agents Endpoint", false, `Unexpected status: ${response.status}`, JSON.stringify(response.data).substring(0, 100));
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
async function testRateLimitDetection() {
    ...
    try {
        ...
        const requests = Array(3)
            .fill(0)
            .map(() => axios.get(`${API_BASE_URL}/user`, {
        ...
        if (rateLimited) {
            recordTest("Rate Limit Detection", true, "Rate limiting detected (429 status) - this is expected behavior");
        }
        else {
            recordTest("Rate Limit Detection", true, "No rate limiting detected in test requests");
        }
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
