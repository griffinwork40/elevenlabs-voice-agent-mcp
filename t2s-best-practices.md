...
<Note>
  Phoneme tags are only compatible with "Eleven Flash v2", "El Turbo v2" and "Eleven English v1"
...
```xml Incorrect usage
<phoneme alphabet="cmu-arpabet" ph="P R AH N AH N S IY EY SH AH N pronunciation
</phoneme>
```
...
To generate a pronunciation dictionary `.pls` file, there are a few open source tools available:
    ...
    - [CMU Pronounc Dictionary](https://github.com/cmusphinx/cmudict) - A pre-built English dictionary with phonetic transcriptions.
  ...
  <Accordion title="Common Issues">
    <ul>
      ...
      <li>
        ...
        <strong>Remember to remove y emotional guidance text in post-production.</strong>
      </li>
    </ul>
  </Accordion>
...
<Steps>
    ...
    <Step title="Handle different number formats">
        ...
        - Roman numerals: "XIV" → "fourteen" (or "the fourte)
    </Step>
    ...
    <Step title="Consider edge cases">
        ...
        - Dates: "01/02/202cond, twenty twenty-three" or "the first of February, twenty twenty-three" (depending on locale)
...
"⅔" → "two-thirds"
"Dr." → "D"St." → "Street" (but saints like "St. Patrick" should remain)
"Ctrl + Z" → "control z"
...
# Initialize inflect engine fconversion
p = inflect.engine()
...
def normalize_text(text: str) -> str:
    ...
    text = re.sub(r"([$£€¥])(\d3})*(?:\.\d{2})?)", money_replacer, text)
...
function normalizeText(text: string): string {
  return (
    text
      // Convert monetary value.g., "$1000" → "one thousand dollars", "£1000" → "one thousand pounds")
...
This guide provides the most effective tags and techniques for prompting Eleven v3, including voice selection, changes ipitalization, punctuation, audio tags and multi-speaker dialogue. Experiment with these methods to discover what works best for your specific voice and use case.
...
The stability slider is the mo important setting in v3, controlling how closely the generated voice adheres to the original reference audio.
...
<Note>
  ...
  with certain voices while others may not. Don't pect a whispering voice to suddenly shout with a
  `[shout]` tag.
</Note>
...
<Warning>
  Some experimental tags may be less consistent across different voices. Test troughly before
  production use.
</Warning>
...
<Tabs>
  <Tab title="Expressive monologue">
   ...
   nd it wasn't even for the story, initially.
  ...
  <Tab title="Customer service simulation">
    ...
    [questioni] "Okay, could you tell me a little more about what you're seeing on the screen?"
...
<Tabs>
  <Tab title="Dialogue showcase">
    ...
    Speaker 1: [delightet's so much better than our old "ha. ha. ha." robot chuckle!
  ...
  <Tab title="Glitch comedy">
    ...
    Speaker 1: [desperately] Two, this isn't funny! I have a prestion in an hour and I sound like a dial-up modem!
  ...
  <Tab title="Overlapping timing">
    ...
    Speaker 1: [mischievously] Race youentence!
